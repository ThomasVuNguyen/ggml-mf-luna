...
int main() {
    ...
    // Get model's hidden size directly
    int64_t hidden_size = model.get_hidden_size();
    if (hidden_size > 0) {
        std::cout << "Model hidden size: " << hidden_size << std::endl;
    } else {
        std::cout << "Failed to retrieve model hidden size" << std::endl;
    }
    ...
    if (!sample_embedding.empty()) {
        ...
        std::cout << "Embedding matrix size: " << embedding_dim << " x " << vocab_size;
